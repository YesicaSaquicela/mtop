...
public class AccessRewriteConfiguration extends HttpConfigurationProvider implements NonEnriching {
    ...
    public Configuration getConfiguration(final ServletContext context) {
        return ConfigurationBuilder.begin()
                .addRule(Join.path("/").to("/paginas/inicio.xhtml"))
                
                //.addRule(Join.path("/signup").to("/paginas/signup.xhtml"))
                .addRule(Join.path("/login").to("/paginas/login.xhtml"))
                // 404 and Error
                .addRule(Join.path("/404").to("/paginas/404.xhtml").perform(Response.setCode(404)))
                .addRule(Join.path("/error").to("/paginas/error.xhtml"))
                // Authentication
                //               .defineRule()
